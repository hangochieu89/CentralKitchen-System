...
<!-- ── PAGE: INVENTORY ── -->
  <div id="page-inventory" style="display:none;">
    <div class="page-header">
      ...
      <div class="actions">
        <button class="btn btn-primary" onclick="document.getElementById('dlg-input').showModal()">+ Nhập kho</button>
      </div>
    </div>

    <div class="filter-bar" style="border:1px solid var(--gray-100);border-radius:var(--radius);background:var(--white);margin-bottom:0;">
      <input type="search" placeholder="Tìm tên sản phẩm…" style="width:240px;" />
      ...
      <select><option>Tất cả mức tồn</option><option>Bình thường</option><option>Thấp (&lt; min)</option></select>
    </div>
        ...
        <c:otherwise>
          ...
          <article class="inv-card ${inv.quantity < inv.minThreshold ? 'low' : 'ok'}">
            ...
            <div style="display:flex;gap:6px;margin-top:4px;">
              <button class="btn btn-ghost btn-sm" style="flex:1;">Chi tiết</button>
            </div>
          </article>
          </c:forEach>
        </c:otherwise>
